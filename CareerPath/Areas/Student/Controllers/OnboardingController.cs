...
//
//     // -------------------------------------------------------------------------
//     // POST /Student/Onboarding/Analyze
//     // Receives resume + githubUsername
//     // Calls /partial-analysis → generates questions
//     // Returns step 2 (loading) → auto-transitions to step 3 (questionnaire)
//     // -------------------------------------------------------------------------
//     [HttpPost]
...
//         try
//         {
//             // Call /partial-analysis — only purpose here is to generate questions
//             var result = await _pythonApiService.PartialAnalysisAsync(resume, githubUsername,studentId);
//
//             // Persist partial profile so we know the student started onboarding
//             var profile = await _unitOfWork.StudentProfile.GetByStudentId(studentId);
//             if (profile == null)
//             {
...
//             }
//
//             // Save partial feature vector so we have something even if step 3 fails
//             profile.GitHubUsername  = JsonSerializer.Serialize(result?.AnalysisResponse);
//             profile.LastAnalyzedAt = DateTime.UtcNow;
//             await _unitOfWork.SaveAsync();
//
//             // Pass githubUsername through so the step 3 form can include it
//             return View("Index", new OnboardingViewModel
//             {
//                 CurrentStep    = 2,
//                 GitHubUsername = githubUsername,
...
//             {
//                 CurrentStep  = 1,
//                 ErrorMessage = $"Analysis failed: {ex.Message}"  // show it in dev
//             });
//         }
//     }
//
...
//     // Receives: resume (file) + githubUsername + answers dictionary
//     // Calls /full-analysis with everything in one shot
//     // Persists final feature vector and marks onboarding complete
//     // -------------------------------------------------------------------------
//     [HttpPost]
//     [ValidateAntiForgeryToken]
...
//         try
//         {
//             // Call /full-analysis with resume + github + questionnaire answers
...
//             }
//
//             // Persist final feature vector and mark onboarding complete
//             profile.GitHubUsername       = JsonSerializer.Serialize(result);
